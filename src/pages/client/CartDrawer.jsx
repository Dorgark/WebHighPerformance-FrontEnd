...
export default function CartDrawer() {
    ...
    return (
        <div className="min-h-screen bg-gray-50 font-sans flex flex-col">
            ...
            {/* ── HEADER ── */}
            <div className="flex items-center gap-4 px-5 pt-14 pb-5 bg-white shadow-sm lg:pt-6 lg:px-10 lg:max-w-7xl lg:mx-auto lg:w-full">
                ...
                {itens.length > 0 && (
                    <span className="ml-1 bg-[#ea580c] text-white text-xs font-bold px-2 py-0.5 rounded-full">
                ...
                <main className="flex-1 overflow-y-auto px-4 py-4 pb-40">
                    {itens.length === 0 ? (
                        ...
                        <div className="flex flex-col gap-4">
                            {itens.map((item) => (
                                <CartItem key={getId(item)} item={item} />
                            ))}
                        </div>
                    )}
                </main>
            ...
            {/* ── DESKTOP LAYOUT ── */}
            <div className="hidden lg:flex flex-1 w-full max-w-7xl mx-auto px-10 py-8 gap-8 items-start">
                ...
                <main className="flex-1">
                    {itens.length === 0 ? (
                        ...
                        <>
                            <h2 className="text-gray-500 text-sm font-semibold uppercase tracking-widest mb-4">
                                {itens.length} {itens.length === 1 ? "produto" : "produtos"} no carrinho
                            </h2>
                            <div className="flex flex-col gap-4">
                                {itens.map((item) => (
                                    <CartItem key={getId(item)} item={item} />
                ...
                {/* Sidebar de resumo */}
                {itens.length > 0 && (
                    <aside className="w-[360px] flex-shrink-0 sticky top-8">
                        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                            {/* Cabeçalho do resumo */}
                            <div className="bg-[#D83D00] px-6 py-4">
                            ...
                            <div className="px-6 py-5 flex flex-col gap-4">
                                ...
                                <div className="flex flex-col gap-2">
                                    {itens.map((item) => (
                                        <div key={getId(item)} className="flex justify-between items-center text-sm text-gray-600">
                                            <span className="line-clamp-1 flex-1 pr-2">
                                                {item.name}
                                                <span className="text-gray-400 ml-1">x{item.quantidade}</span>
                                            </span>
                                ...
                                <div className="border-t border-gray-100 pt-4">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <span className="text-gray-800 font-semibold">R$ {totalPreco.toFixed(2)}</span>
                                    </div>
                                    <div className="flex justify-between items-center mt-2">
                                        <span className="text-gray-500 text-sm">Retirada no balcão</span>
                                ...
                                <div className="border-t border-gray-100 pt-4 flex justify-between items-center">
                                    ...
                                    <span className="text-[#ea580c] font-black text-2xl">
                                ...
                                <button
                                    onClick={finalizarPedido}
                                    className="w-full bg-[#25D366] text-white font-bold py-4 rounded-xl flex items-center justify-center gap-3 text-base hover:brightness-110 active:scale-95 transition-all shadow-md mt-1"
                                >
                                    <WhatsappIcon />
                                    Finalizar pedido!
                                </button>
                                ...
                                <button
                                    onClick={() => navigate("/")}
                                    className="w-full text-center text-sm text-gray-400 hover:text-orange-600 transition-colors py-1"
                                >
                                    ← Continuar comprando
                                </button>
                            </div>
