...
export default function CartDrawer() {
    ...
    const whatsappNumero = "5581999999999"; // ← troque pelo número real
    function finalizarPedido() {
        if (itens.length === 0) return;

        const linhas = itens.map(
            (i) =>
                `• ${i.name} x${i.quantidade} — R$ ${(parseFloat(i.price) * i.quantidade).toFixed(2)}`
        );
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex justify-between items-start">
                                        <p className="text-sm font-semibold text-gray-800 leading-tight line-clamp-2 pr-2">
                                            {item.name}
                                        </p>
                                        {/* Botão deletar */}
                                        <button
